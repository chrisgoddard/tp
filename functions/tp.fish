...
function _tp_cleanup_ssh_source_hook --description "Clear an outside-attach SSH source hook and its pending values"
    ...
    tmux set-option -guq "@tp_ssh_source_pending_$hook_index" >/dev/null 2>&1
    tmux set-option -guq "@tp_ssh_tty_pending_$hook_index" >/dev/null 2>&1
end
...
function _tp_prepare_ssh_source_for_attach --description "Update or preserve the current client's SSH source"
    ...
    set -l pending_source "@tp_ssh_source_pending_$hook_index"
    set -l pending_tty "@tp_ssh_tty_pending_$hook_index"
...
function _tp_attach --description "Attach or switch to a tmux session"
    ...
    if set -q TMUX
        ...
        _tp_cleanup_ssh_source_hook $ssh_source_hook
